    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div id="qr-reader" style="width: 500px; max-width: 90%; margin: 0
auto;"></div>
...
<script
src="[https://unpkg.com/html5-qrcode/minified/html5-qrcode.min.js](https://unpkg.com/html5-
qrcode/minified/html5-qrcode.min.js)"></script>
<script>
...
resultContainer.innerHTML = `<span class="text-blue-600">Memvalidasi kode
${decodedText}...</span>`;
...
class="text-green-600">${result.message}</span>`;
} else {

// Error (voucher tidak valid, sudah dipakai, dll)
resultContainer.innerHTML = `<span
class="text-red-600">${result.error}</span>`;
}
} catch (error) {
console.error('Fetch error:', error);
resultContainer.innerHTML = `<span class="text-red-600">Error: Gagal
terhubung ke server.</span>`;
}
// Tambahkan tombol untuk scan lagi
resultContainer.innerHTML += ` <button onclick="window.location.reload()"
class="ml-2 text-sm text-blue-500 underline">Scan Lagi</button>`;
}
});
</script>
</x-app-layout>