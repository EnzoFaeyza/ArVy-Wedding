...
'error' => "Voucher sudah digunakan pada {$voucher->used_at} oleh tamu
{$voucher->guest->name}."
...
$voucher->save();
Log::info("Voucher redeem SUCCESS: {$code} for guest
{$voucher->guest->name}");
return response()->json([
'message' => "Voucher BERHASIL ditukar! Diskon 10% untuk Tamu:
{$voucher->guest->name}"
], 200);
} catch (\Exception $e) {
Log::error("Voucher redeem ERROR on save: {$code} - " . $e->getMessage());
return response()->json(['error' => 'Terjadi kesalahan server saat menyimpan data.'],
500);
}
}
}
